...
@section('title', 'Unggah dokumen akreditasi')
@section('heading', 'Unggah dokumen akreditasi')
...
<div class="w-full min-w-0 space-y-6">
    ...
    <div class="mx-auto max-w-2xl rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-xl shadow-slate-900/[0.04] ring-1 ring-white/70 backdrop-blur-sm md:p-8">
        <form method="post" action="{{ route('admin.dokumen-akreditasi.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @php
            ...
            <div>
                <label for="pdf" class="{{ $lbl }}">Berkas PDF</label>
                <input id="pdf" type="file" name="pdf" accept=".pdf,application/pdf" class="block w-full text-sm text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-primary file:px-4 file:py-2 file:text-xs file:font-bold file:text-white @error('pdf') border border-rose-400 rounded-lg @enderror" required>
                <p class="mt-1 text-xs text-slate-500">Maksimal 20 MB.</p>
