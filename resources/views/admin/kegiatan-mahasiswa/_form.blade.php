@php
    /** @var \App\Models\StudentActivity $activity */
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <div class="sm:col-span-2">
            ...
            <input id="sort_order" type="number" name="sort_order" min="0" max="65535" required value="{{ old('sort_order', $activity->sort_order ?? 0) }}"
                ...
                aria-describedby="sa-sort-help">
            <p id="sa-sort-help" class="mt-2 rounded-xl border border-sky-100 bg-sky-50/80 px-3 py-2 text-[11px] leading-relaxed text-slate-600">
                Angka <strong>lebih kecil</strong> dipajang lebih awal pada grid kegiatan mahasiswa.
            </p>
            @error('sort_order')
    ...
    <div class="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3">
        <label class="flex cursor-pointer items-start gap-3">
            ...
            <span>
                ...
                <span class="mt-0.5 block text-[11px] leading-relaxed text-slate-600">Nonaktifkan untuk menyimpan sebagai draf; entri draf tidak muncul di halaman <code class="rounded bg-white px-1 font-mono text-[10px]">/kegiatan-mahasiswa</code>.</span>
            </span>
        </label>
        @error('is_published')
