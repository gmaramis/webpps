...
@section('title', 'Ubah halaman Stop Korupsi')
@section('heading', 'Ubah halaman Stop Korupsi')
...
<div class="w-full min-w-0 space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-2 text-sm text-slate-600">
        ...
        <a href="{{ route('stop-korupsi') }}" target="_blank" rel="noopener noreferrer" class="inline-flex shrink-0 items-center rounded-full border border-slate-200/90 bg-white px-3 py-1.5 text-xs font-bold text-primary shadow-sm transition hover:shadow-md">Lihat hasil ↗</a>
    </div>
    ...
    @else
        @php /** @var \App\Models\StopKorupsiPageContent $page */ @endphp
        @php
        ...
        <div class="mx-auto max-w-2xl rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-md ring-1 ring-white/70 md:p-5">
            <p class="mb-4 text-sm text-slate-600">Isian di bawah untuk tampilan pengunjung di halaman <strong class="text-slate-900">Stop Korupsi</strong>. Kosongkan suatu kotak bila ingin memakai teks bawaan situs.</p>

            <form method="post" action="{{ route('admin.stop-korupsi.konten.update') }}" class="space-y-5">
                @csrf
                @method('PATCH')
                ...
                <fieldset class="space-y-4 rounded-xl border border-slate-100 bg-slate-50/40 p-4">
                    ...
                    <div>
                        <label for="link_span_lapor_url" class="{{ $lbl }}">Alamat yang dibuka tombol SPAN <span class="font-normal text-slate-500">(boleh kosong)</span></label>
                        <input id="link_span_lapor_url" type="text" name="link_span_lapor_url" value="{{ old('link_span_lapor_url', $page->link_span_lapor_url) }}" placeholder="https://…" class="{{ $in }} font-mono text-xs @error('link_span_lapor_url') border-rose-400 @enderror">
                        @error('link_span_lapor_url')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                    </div>
                </fieldset>
                ...
                <div class="flex flex-wrap gap-2 border-t border-slate-100 pt-4">
                    ...
                    <a href="{{ route('admin.stop-korupsi.hub') }}" class="rounded-full border border-slate-200 bg-white px-5 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">Batal</a>
                </div>
            </form>
        </div>
    @endif
