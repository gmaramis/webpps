...
@section('title', 'Edit poin Stop Gratifikasi')
@section('heading', 'Stop Gratifikasi — edit poin')
...
<div class="w-full min-w-0 space-y-4">
    <nav class="flex flex-wrap gap-2 text-sm text-slate-600">
        ...
        <span>/</span>
        <a href="{{ route('admin.stop-gratifikasi.poin.index') }}" class="font-semibold text-primary hover:underline">Poin</a>
        <span>/</span>
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-lg ring-1 ring-white/70 md:p-5">
        <form method="post" action="{{ route('admin.stop-gratifikasi.poin.update', $bullet) }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-wrap gap-2 border-t border-slate-100 pt-3">
                ...
                <a href="{{ route('admin.stop-gratifikasi.poin.index') }}" class="rounded-full border border-slate-200 px-5 py-2 text-sm font-semibold text-slate-700">Batal</a>
            </div>
        </form>
    </div>
</div>
