...
@section('title', 'Program S2 (Magister)')
@section('heading', 'Program S2 (Magister)')
...
<div class="w-full min-w-0 space-y-5">
    <nav class="flex flex-wrap items-center gap-2 text-sm text-slate-600" aria-label="Jejak navigasi">
        ...
        <span class="font-medium text-slate-900">Program S2</span>
    </nav>
    ...
    <div class="flex flex-col gap-3 rounded-3xl border border-slate-200/80 bg-white/90 px-5 py-4 shadow-lg shadow-slate-900/[0.04] ring-1 ring-white/70 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
            <p class="text-sm text-slate-600">Data menampil halaman publik <code class="rounded-md bg-slate-100 px-1.5 py-0.5 text-xs font-mono">/s2</code>. Pengunjung memilih program (parameter URL <code class="rounded bg-slate-100 px-1 font-mono">program</code>) lalu melihat deskripsi singkat dan tautan situs resmi prodi.</p>
            @if($programs->isEmpty())
        ...
        <div class="flex flex-wrap gap-2">
            @if($ppsJsonExists)
                <form method="post" action="{{ route('admin.prodi-s2.import-json') }}" class="inline" onsubmit="return confirm('Ini akan menghapus semua program S2 di basis data dan menggantinya dari key PROGRAMS_MAGISTER di resources/data/pps-content.json. Lanjut?');">
                    @csrf
            ...
            @endif
            <a href="{{ route('program.s2') }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center rounded-full border border-slate-200/90 bg-white px-4 py-2 text-xs font-bold text-primary shadow-sm transition hover:shadow-md">Halaman publik ↗</a>
            <a href="{{ route('admin.prodi-s2.create') }}" class="inline-flex items-center rounded-full bg-gradient-to-r from-primary to-primary-light px-4 py-2 text-xs font-bold text-white shadow-lg shadow-primary/20 transition hover:brightness-110">+ Tambah program</a>
        </div>
    </div>
    ...
    <div class="overflow-hidden rounded-3xl border border-slate-200/80 bg-white/90 shadow-xl shadow-slate-900/[0.04] ring-1 ring-white/70 backdrop-blur-sm">
        @if($programs->isEmpty())
            <p class="px-6 py-16 text-center text-sm text-slate-500">Belum ada program S2.</p>
        @else
                        ...
                        @foreach($programs as $p)
                            <tr class="transition hover:bg-slate-50/80">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 pr-6 text-right">
                                    <a href="{{ route('admin.prodi-s2.edit', $p) }}" class="mr-2 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-primary transition hover:bg-sky-100">Edit</a>
                                    <form method="post" action="{{ route('admin.prodi-s2.destroy', $p) }}" class="inline" onsubmit="return confirm('Hapus program ini?');">
                                        @csrf
                                        @method('DELETE')
