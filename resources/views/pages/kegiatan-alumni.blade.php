...
@section('title', ($t['alumniActivitiesTitle'] ?? 'Kegiatan Alumni').' — '.($t['brandTitle'] ?? 'PPS UNIMA'))

@php
    $loc = app()->getLocale();
    $items = $ppsData['ALUMNI_ACTIVITIES'] ?? [];
@endphp
...
<main id="main" class="pb-16 pt-6 md:pb-20 md:pt-8">
    <div class="mx-auto max-w-6xl px-4">
        <header class="activities-page-head-reveal mb-10 border-b border-slate-200 pb-8">
            <p class="text-xs font-bold uppercase tracking-widest text-primary">{{ $t['alumniActivitiesEyebrow'] }}</p>
            <h1 class="mt-2 font-display text-3xl font-bold tracking-tight text-primary md:text-4xl">{{ $t['alumniActivitiesTitle'] }}</h1>
            <p class="mt-4 max-w-3xl text-base leading-relaxed text-slate-600 md:text-lg">{{ $t['alumniActivitiesLead'] }}</p>
        </header>

        <p class="activities-grid-label-reveal mb-4 text-sm font-medium text-slate-700">{{ $t['alumniActivitiesGridLabel'] }}</p>
