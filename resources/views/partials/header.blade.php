...
<div class="site-header-wrap sticky top-0 z-[100] border-b border-black/20 shadow-md shadow-black/20">
        ...
        <div class="mx-auto max-w-6xl px-4">
            <div class="flex items-center justify-between gap-3 py-3 md:gap-4 md:py-3.5">
                ...
                <div class="flex shrink-0 items-center gap-2 sm:gap-3">
                    ...
                    <button type="button" id="mobile-menu-toggle" class="nav-top-link inline-flex items-center justify-center rounded-lg px-3 py-2 text-sm font-medium text-white/90 transition hover:bg-white/10 hover:text-white md:hidden" aria-expanded="false" aria-controls="mobile-menu" aria-label="{{ $t['navMenuToggle'] }}">
    ...
    <div class="site-header site-header-nav-strip">
        <div class="mx-auto max-w-6xl px-4">
            <nav class="hidden py-2 md:block" aria-label="{{ $t['navAriaMain'] }}">
                <ul class="m-0 flex list-none flex-wrap items-center gap-x-0.5 gap-y-1 p-0 md:gap-x-1">
                    @foreach($ppsData['NAV'] ?? [] as $item)
                        @if(isset($item['children']))
                            <li class="group relative">
                                <button type="button" class="nav-top-link flex w-full items-center gap-1 rounded-lg px-3 py-2 text-left text-sm font-medium text-white/90 transition hover:bg-white/10 hover:text-white lg:w-auto lg:justify-center" aria-expanded="false" aria-haspopup="true">
                                ...
                                <ul role="menu" class="absolute left-0 top-full z-[200] mt-0 hidden min-w-[15rem] rounded-lg border border-white/30 bg-[#001a99] py-2 shadow-xl ring-1 ring-black/45 group-hover:block group-focus-within:block">
            ...
            <div id="mobile-menu" class="nav-mobile-panel hidden max-h-[min(70vh,calc(100dvh-8rem))] overflow-y-auto md:hidden">
